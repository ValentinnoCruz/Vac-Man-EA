...
import random
import pygame
pygame.init() # Pygame is used for the reproduction of sounds
...
class PacmanArena(Arena):
    ...
    def __init__(self, width: int, height: int):
        ...
        ## Sounds
        self._sounds = [pygame.mixer.Sound('sound/Dies.wav'),
    ...
    def playing(self) -> bool:
        ...
        return result # 0 -> contine, 1 -> Victory, 2 -> Game over
    ...
    def double_ghost(self):
        ghosts = self._actors[:]
        for a in ghosts:
            if isinstance(a, Ghost):
                Ghost(self, a._x - a._dir[0], a._y - a._dir[1], a._color)
...
class Ghost(Actor):
    ...
    def status(self, status: int):
        ...
        ## 1 -> Pac-Man lose life
        ## 2 -> Power Pac-Man
        ## 3 -> Power Pac-Man in conclusion
        ## 4 -> Eatten by PacMan during state 2/3
        ...
        elif status == 4: self._arena.remove(self)
        self._status = status
        self._counter = 0

    def pos_init(self): # The ghost's position is reset to its initial position
        self._x, self._y = self._start_x, self._start_y
        self._sprite = [64, 64 + self._color*16]

    def start(self): # The ghost begins to move
        self._speed = 2
        self._dir = [0, -2]
        self._sprite = [64, 64 + self._color*16]

    def normal(self): # The ghost returns to normal speed
        self._speed = 1
        if self._y == 112 and 92 <= self._x <= 128: self._gate = True # If he is inside the initial enclosure he must be able to exit
    ...
    def stop(self): # The ghost stops
        self._speed = 0
        self._dir = [0, 0]

    def runaway(self): # The ghost reverses, turns blue, and slows down
        self._speed = 1
    ...
    def move(self):
        ...
        ## The ghost only retrofits if it can't do anything else
        if len(dirs) == 0: self._dir = [-self._dir[0], -self._dir[1]]
        ## Frightened mode (random movements)
        elif self._status in [2, 3]: self._dir = random.choice(dirs)
        else:
            ## Target selection (xt, yt)
            if self._gate and self._status != 4: # The ghost exits the initial enclosure to enter the game
                xt, yt = 108, 88
                if self._x == xt and self._y == yt:
                    self.status(0)
                    self._gate = False
            elif self._status == 4: #The ghost returns to the initial enclosure when eaten by PacMan
                xt,yt = 108, 112
                if self._x == xt and self._y == yt:
                    self.status(0)
                    self._gate = True
            elif self._behav == 0: # Scatter mode (corner patrol)
                xt,yt = angles[self._color]
                if self._behav_count == FRAME_RATE*7:
                    self._behav_count = 0
                    self._behav = 1
                else: self._behav_count += 1
            elif self._behav == 1: # Chase mode (Pac-Man chase)
                for a in self._arena.actors():
            ...
            else: # Calculation of the shortest distance to reach the goal
                x, y = self._x, self._y
                distances = []
        ...
        ## Finally, the movement is carried out
        self._x += self._dir[0]
        self._y += self._dir[1]
...
class PacMan(Actor):
    ...
    def __init__(self, arena, x:int, y:int):
        ...
        self.scores_sprite = []
        self.bonus_sprite = []
        self._bonus = [100, 300, 500, 700, 1000, 2000, 3000, 5000] #bonus values
        self._gate = False
    ...
    def pos_init(self): # PacMan's position is reset to its initial position
        self._x, self._y = 108, 184
        self._sprite = (16, 16)

    def start(self): # PacMan starts moving
        self.pos_init()
        self._dir = (-2, 0)
        self._next_dir = (-2, 0)

    def stop(self): # PacMan stops
    ...
    def collide(self, other):
        if isinstance(other, Wall):
            ...
            if (y < self._y + self.H//2 and self._y < y + h//2 and x < self._x + self.W//2 and self._x < x + w//2):
                ...
                # el
                if other.getStatus() == 0 or other.getStatus() == 2 or other.getStatus() == 3:
                    self._arena.sound(5).play()
                    ## Calculation of bonus points
                    num = 0 # Number of ghosts already eaten
                    for a in self._arena.actors():
    ...
    def move(self):
        ...
        if self._status == 0:
            ...
            ## Replacing dir with next_dir if possible
